...
rnaPCs <- 30
adtPCs <- 18
n <-17    # Let variables reach up to n GB
...
wkdir <- getwd()
list <- list()
rds_files <- list.files(path = paste0(wkdir,"/"), pattern = paste0("\\-", fileType, "_Seurat.rds$"), full.names = TRUE)
...
sweep.res.list <- paramSweep(combinedRNA, PCs = 1:rnaPCs, sct = TRUE)
...
print(DimPlot(combinedRNA, reduction = 'rna.umap', group.by = "DF.classifications_0.25_0.21_691"))
dev.off()
write.csv(table(combinedRNA@meta.data$orig.idents), file=paste0(project, "-", fileType, "_doublets.csv"))
saveRDS(combinedRNA, file = paste0(project,"-", fileType, "_doubletsFound_integrated.RDS"))
combinedRNA <- subset(combinedRNA, subset = combinedRNA$somethingomething == 'Singlet')
saveRDS(saveRDS(combinedRNA, file = paste0(project,"-", fileType, "_singlets_integrated.RDS")))
...
for (res in resRange) {
  rds <- FindClusters(rds, resolution = res, verbose = FALSE)
...
rds <- RunUMAP(rds, reduction = "pca", reduction.name = "wnn.umap", 
               reduction.key = 'wnnUMAP_', dims = 1:rnaPCs, n.components = 3L)
Embeddings(object = rds, reduction = "wnn.umap")
