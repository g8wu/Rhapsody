...
DimPlot(rds, reduction="harmony_umap", group.by="scDblFinder_class", cols=c('grey', 'red'), order=TRUE)
FeaturePlot(rds, reduction="harmony_umap", features='scDblFinder_score', raster = F, order=TRUE)
...
# Save summary
write.csv(data.frame(prop.table(table(rds$scDblFinder_class))*100), paste0(project, "-DbF.csv"), row.names = F)
...
write.table(data.frame(Class = "Singlet", 
            ...
            paste0(project, "-DblFinder.csv"), sep = ",", row.names = F, col.names = F, append = T)
# Save
saveRDS(rds, paste0(project, "Norm-BeC-DbF.rds"))
...
rds$seurat_clusters <- rds$wsnn_res.0.2
